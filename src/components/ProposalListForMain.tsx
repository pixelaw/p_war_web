      ...
      <div className={`overflow-y-auto px-2`} style={{ height: `calc(100vh - ${headerHeight}px - 170px)` }}>
        <div className='space-y-4'>
          {filteredProposals.map((proposal, index) => {
            ...
            return (
              <div key={index} className={`relative bg-opacity-50 p-4 rounded-md border border-gray-700 hover:border-gray-600 transition-colors duration-300 ${getStatusColorForBg(proposal.status)}`}>
                {/*<div key={index} className={`relative bg-opacity-50 p-4 mb-4 rounded-lg ${getStatusColor(proposal.status)}`}> */}
                <div className='block'>
                  <div className='flex justify-between items-center mb-1'>
                    ...
                    <div className={`px-2 py-1 rounded-md text-white text-xs ${getStatusColor(proposal.status)}`}>
                      {proposal.status.startsWith('end in') ? proposal.status.replace('end in ', '')  : 'closed'}
                    </div>
                  </div>
                  ...
                  <div className='flex justify-between text-xs text-gray-300 mr-20'>
                    <div>
                      For {proposal.forPoints}pt.
                    </div>
                    <div>
                      Against {proposal.againstPoints}pt.
                    </div>
                  </div>
                </div>
                <button 
